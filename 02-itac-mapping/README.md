...
| **빅4 메소돌로지 일반화 (CACIR)** | 5영역: Configuration / Authorization / Calculation / Interface / Report(IPE) | 빅4 내부 메소돌로지마다 약간씩 다름. 표준화된 명칭 X |
...
**본 레포의 입장**: 빅4 5영역 분류(CACIR)를 reference로 사용. 다만 한국 비금융 회계감사 실무에서 자주 마주치는 통제는 프로세스 단위로 정리.
...
| ISMS-P 인증기준 | ITAC 영역 (CACIR) | 매핑 근거 |
|---|---|---|
| 2.6.3 응용프로그램 접근 | Authorization | 응용 시스템 사용자별 화면·기능·메뉴 권한 통제 |
| 2.6.4 데이터베이스 접근 | Authorization | 응용 시스템 → DB 데이터 접근 통제 |
| 2.8.1 보안 요구사항 정의 | Configuration | 응용 시스템 설계 단계 보안 통제 요구사항 |
| 2.8.2 보안 요구사항 검토 및 시험 | Configuration · Validation | 자동통제 설정값 검토 |
| 2.8.3 시험과 운영 환경 분리 | (간접) | ITAC 검증 환경의 신뢰성 전제 |
| 2.8.5 소스 프로그램 관리 | (간접) | ITAC 코드 변경 통제 — ITGC와 겹침 |
| 2.8.6 운영환경 이관 | (간접) | ITAC 변경 → 운영 반영 통제 — ITGC와 겹침 |
| 2.9.1 변경관리 | (간접) | ITAC 변경 통제 — ITGC와 겹침 |
| 2.10.4 전자거래 및 핀테크 보안 | Validation · Calculation | 전자거래 트랜잭션 정합성 |
...
ITAC 매핑은 ITGC와 다르게 다음 순서로 작성:

1. **프로세스별 자동통제 카탈로그** — 매출/매입/재고/결산/HR 5개 프로세스별로 빈출 자동통제 정리
2. **각 통제의 5영역 분류 (CACIR)** — Configuration / Authorization / Calculation / Interface / Report
